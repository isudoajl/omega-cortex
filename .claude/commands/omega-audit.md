...
**Only when `--fix` is present.** Read-only audits produce no code changes, no commits, and no gate interactions — tracking is overhead with no value.

If `--fix` AND `.claude/memory.db` exists, register this workflow run:
...
**Without `--fix`:** Skip pipeline tracking entirely. The audit report artifact is sufficient.
